...
The video is NOT official broadcast footage — it is a stylized dramatic edit
(cartoon, comic, paint, neon, anime, watercolor, pixel, noir, thermal, vapor, and others)
with a motion pack (trailer, smash, freeze, replay, whip, panels, drift, strobe, glitch, or impact).

Return a JSON object with exactly these keys:
- title: string, max 100 characters, punchy, no clickbait spam, include the main subject
- description: one single line of text, no newline characters. Hook, then a short recut note,
  then 6–10 hashtags at the end of the same line.
...
def _facts_block(info: dict) -> str:
    ...
    if info.get("pack"):
        lines.append(f"motion_pack: {info['pack']}")
...
def _parse(text: str) -> dict:
    ...
    description = _one_line(str(data.get("description") or ""))
...
def _fallback(info: dict) -> dict:
    ...
    title = title[:100]
    tags = []
    for t in (genre, player, teams, moment, about, info.get("style") or "animated",
              info.get("pack") or "cartoon", "shorts", "dramatic", "highlight", "recut", "sports"):
    ...
    description = _one_line(
        f"{about}. Animated {genre} recut — {info.get('style') or 'cartoon'} look, "
        f"{info.get('pack') or 'dramatic'} edit. Stylized edit of footage you provided. "
        f"Not official broadcast. Like if you want more of these. {hashes}"
    )
    return {
        "title": title,
...
def generate(info: dict, *, require_llm: bool = False) -> dict:
    ...
    try:
        ...
        out = _parse(raw)
        out["source"] = backend
        out["description"] = _one_line(out["description"])
        return out
